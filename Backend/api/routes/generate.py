    ...
    if(os.getenv("JIRA_MCP_ENABLED", "false").lower() == "true"):
        ...
        try:
            ...
            jira_prompt = (
                ...
                f"Push the artifacts below to Jira project '{jira_project_key}' using four sequential "
                f"jira_batch_create_issues calls (Epics → Features → Use Cases → Test Cases).\n"
                f"Parent keys from each step are required as input to the next step, so the calls MUST be sequential.\n\n"
                f"RULES\n"
                f"  - Every MCP tool call argument must be valid JSON matching the function signature exactly.\n"
                f"  - No Python code, no comments, no explanations inside tool arguments.\n"
                f"  - All reasoning must be internal — do NOT narrate steps; call tools immediately.\n"
                f"  - Use ONLY jira_batch_create_issues. Never call jira_create_issue.\n"
                f"  - Priority mapping: 'Critical' → 'Highest' | 'High' → 'High' | 'Medium' → 'Medium' | 'Low' → 'Low'.\n"
                f"  - If a required field is missing or the payload is invalid, return: {{\"error\": \"invalid_payload\"}} and stop.\n\n"
                f"ARTIFACTS:\n{json.dumps(messages)}\n\n"
                f"WORKFLOW\n\n"
                f"STEP 1 — Call jira_batch_create_issues with this exact argument shape:\n"
                f"  {{\n"
                f"    \"project_key\": \"{jira_project_key}\",\n"
                f"    \"issues\": [\n"
                f"      {{ \"summary\": \"<epic name>\", \"description\": \"<epic description>\", \"issue_type\": \"Epic\", \"priority\": \"<mapped priority>\" }},\n"
                f"      ... one entry per epic\n"
                f"    ]\n"
                f"  }}\n"
                f"  From the response, record jira_issue_key, jira_issue_id, jira_issue_url for each epic.\n\n"
                f"STEP 2 — Call jira_batch_create_issues after Step 1 completes:\n"
                f"  {{\n"
                f"    \"project_key\": \"{jira_project_key}\",\n"
                f"    \"issues\": [\n"
                f"      {{ \"summary\": \"<feature name>\", \"description\": \"<feature description>\", \"issue_type\": \"New Feature\", \"epic_link\": \"<parent epic jira_issue_key from Step 1>\", \"priority\": \"<mapped priority>\" }},\n"
                f"      ... one entry per feature\n"
                f"    ]\n"
                f"  }}\n"
                f"  From the response, record jira_issue_key, jira_issue_id, jira_issue_url for each feature.\n\n"
                f"STEP 3 — Call jira_batch_create_issues after Step 2 completes:\n"
                f"  {{\n"
                f"    \"project_key\": \"{jira_project_key}\",\n"
                f"    \"issues\": [\n"
                f"      {{ \"summary\": \"<use case title>\", \"description\": \"<use case description + acceptance criteria>\\n\\nRelated Feature: <parent feature jira_issue_url from Step 2>\", \"issue_type\": \"Improvement\", \"priority\": \"<mapped priority>\" }},\n"
                f"      ... one entry per use case\n"
                f"    ]\n"
                f"  }}\n"
                f"  From the response, record jira_issue_key, jira_issue_id, jira_issue_url for each use case.\n\n"
                f"STEP 4 — Call jira_batch_create_issues after Step 3 completes:\n"
                f"  {{\n"
                f"    \"project_key\": \"{jira_project_key}\",\n"
                f"    \"issues\": [\n"
                f"      {{ \"summary\": \"<test case title>\", \"description\": \"<preconditions + steps + expected result + compliance>\\n\\nRelated Use Case: <parent use case jira_issue_url from Step 3>\", \"issue_type\": \"Task\", \"priority\": \"<mapped priority>\" }},\n"
                f"      ... one entry per test case\n"
                f"    ]\n"
                f"  }}\n"
                f"  From the response, record jira_issue_key, jira_issue_id, jira_issue_url for each test case.\n\n"
                f"VALIDATION\n"
                f"  Before each tool call verify that project_key is present, issues array is non-empty, and every issue "
                f"has summary, description, and issue_type. If any check fails return {{\"error\": \"invalid_payload\"}} and stop.\n\n"
                f"OUTPUT\n"
                f"  After all four calls succeed, update every artifact with its Jira fields "
                f"(jira_issue_key, jira_issue_id, jira_issue_url, jira_status='pushed') and return ONLY the following "
                f"JSON — no prose, no markdown:\n"
                f"  {{\n"
                f"    \"epics\": [ ...full hierarchy with all Jira fields populated... ],\n"
                f"    \"epics_generated\": <n>,\n"
                f"    \"features_generated\": <n>,\n"
                f"    \"use_cases_generated\": <n>,\n"
                f"    \"test_cases_generated\": <n>,\n"
                f"    \"pushed_to_jira\": true,\n"
                f"    \"status\": \"pushed_to_jira\"\n"
                f"  }}"
            )
